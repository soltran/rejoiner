...
@rem ##########################################################################
@rem
@rem  helloworld-backend startup script for Windows
@rem
@rem ##########################################################################
...
@rem Add default JVM options here. You can also use JAVA_OPTS and HELLOWORLD_BACKEND_OPTS to pass JVM options to this script.
...
@rem Execute helloworld-backend
"%JAVA_EXE%" %DEFAULT_JVM_OPTS% %JAVA_OPTS% %HELLOWORLD_BACKEND_OPTS%  -classpath "%CLASSPATH%" com.google.api.graphql.examples.helloworld.backend.HelloWorldServer %CMD_LINE_ARGS%
...
:fail
rem Set variable HELLOWORLD_BACKEND_EXIT_CONSOLE if you need the _script_ return code instead of
rem the _cmd.exe /c_ return code!
if  not "" == "%HELLOWORLD_BACKEND_EXIT_CONSOLE%" exit 1
exit /b 1
