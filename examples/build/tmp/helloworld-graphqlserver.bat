...
@rem ##########################################################################
@rem
@rem  helloworld-graphqlserver startup script for Windows
@rem
@rem ##########################################################################
...
@rem Add default JVM options here. You can also use JAVA_OPTS and HELLOWORLD_GRAPHQLSERVER_OPTS to pass JVM options to this script.
...
@rem Execute helloworld-graphqlserver
"%JAVA_EXE%" %DEFAULT_JVM_OPTS% %JAVA_OPTS% %HELLOWORLD_GRAPHQLSERVER_OPTS%  -classpath "%CLASSPATH%" com.google.api.graphql.examples.helloworld.graphqlserver.GraphQlServer %CMD_LINE_ARGS%
...
:fail
rem Set variable HELLOWORLD_GRAPHQLSERVER_EXIT_CONSOLE if you need the _script_ return code instead of
rem the _cmd.exe /c_ return code!
if  not "" == "%HELLOWORLD_GRAPHQLSERVER_EXIT_CONSOLE%" exit 1
exit /b 1
