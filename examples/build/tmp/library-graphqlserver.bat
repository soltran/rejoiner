...
@rem ##########################################################################
@rem
@rem  library-graphqlserver startup script for Windows
@rem
@rem ##########################################################################
...
@rem Add default JVM options here. You can also use JAVA_OPTS and LIBRARY_GRAPHQLSERVER_OPTS to pass JVM options to this script.
...
@rem Execute library-graphqlserver
"%JAVA_EXE%" %DEFAULT_JVM_OPTS% %JAVA_OPTS% %LIBRARY_GRAPHQLSERVER_OPTS%  -classpath "%CLASSPATH%" com.google.api.graphql.examples.library.graphqlserver.GraphQlServer %CMD_LINE_ARGS%
...
:fail
rem Set variable LIBRARY_GRAPHQLSERVER_EXIT_CONSOLE if you need the _script_ return code instead of
rem the _cmd.exe /c_ return code!
if  not "" == "%LIBRARY_GRAPHQLSERVER_EXIT_CONSOLE%" exit 1
exit /b 1
