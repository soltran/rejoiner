...
@rem ##########################################################################
@rem
@rem  shelf-backend startup script for Windows
@rem
@rem ##########################################################################
...
@rem Add default JVM options here. You can also use JAVA_OPTS and SHELF_BACKEND_OPTS to pass JVM options to this script.
...
@rem Execute shelf-backend
"%JAVA_EXE%" %DEFAULT_JVM_OPTS% %JAVA_OPTS% %SHELF_BACKEND_OPTS%  -classpath "%CLASSPATH%" com.google.api.graphql.examples.library.shelfbackend.ShelfServer %CMD_LINE_ARGS%
...
:fail
rem Set variable SHELF_BACKEND_EXIT_CONSOLE if you need the _script_ return code instead of
rem the _cmd.exe /c_ return code!
if  not "" == "%SHELF_BACKEND_EXIT_CONSOLE%" exit 1
exit /b 1
